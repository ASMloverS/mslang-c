...
等）均以字节偏移为单位；如需按码点计算，使用 `strings.codepoint_count`。
...
| `has_prefix` | `has_prefix(s, prefix) → bool` | 是否以 prefix 开头 |
| `has_suffix` | `has_suffix(s, suffix) → bool` | 是否以 suffix 结尾 |
| `index` | `index(s, substr) → int` | 第一次出现的字节位置，未找到返回 -1 |
| `last_index` | `last_index(s, substr) → int` | 最后一次出现的字节位置，未找到返回 -1 |
| `count` | `count(s, substr) → int` | 不重叠出现次数 |
| `replace` | `replace(s, old, new_s, n=-1) → str` | 替换前 n 次出现，n=-1 替换全部 |
...
| `is_alpha` | `is_alpha(s) → bool` | 所有码点是否均为字母（Unicode） |
| `is_digit` | `is_digit(s) → bool` | 所有码点是否均为数字（Unicode） |
| `is_space` | `is_space(s) → bool` | 所有码点是否均为空白（Unicode） |
| `codepoint_count` | `codepoint_count(s) → int` | Unicode 码点数（vs `len` 计字节数） |
...
| `write_byte` | `b.write_byte(n: int)` | 追加单个字节（0–255） |
...
strings.last_index(s, substr) → int
```
...
```
strings.replace(s, old, new_s, n=-1) → str
```

从左至右替换 `old` 为 `new_s`，最多替换 `n` 次。`n=-1` 替换全部。
...
```
strings.is_alpha(s) → bool
strings.is_digit(s) → bool
strings.is_space(s) → bool
```
...
用于高效拼接大量字符串，避免 `+=` 的 O(n²) 问题。`write` 和 `write_byte` 均在 O(1)
...
fmt.println(strings.codepoint_count(s))       // 9（码点数）
...
fmt.println(strings.trim_prefix("foobar", "foo"))     // bar
...
| 异常 | 触发条件 |
|---|---|
| `TypeError` | `join` 的 parts 包含非字符串元素；`write_byte` 传入超出 0–255 范围的值 |
