...
```c
static MsValue builtinAbs(MsThread* t, MsValue* args, int argc) {
...
// round(x, n) → 保留 n 位小数
static MsValue builtinRound(MsThread* t, MsValue* args, int argc) {
...
static MsValue builtinPow(MsThread* t, MsValue* args, int argc) {
  ...
  // 两参数：复用 tpPow（T053/T054）
  MsType* ty = msTypeOf(base);
  if (ty->tpPow) return ty->tpPow(base, exp_);
...
static MsValue builtinDivmod(MsThread* t, MsValue* args, int argc) {
