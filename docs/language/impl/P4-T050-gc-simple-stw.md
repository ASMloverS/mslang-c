...
static void sweep(void) {
  ...
  while (cur) {
    if ((cur->gcFlags & 0x03) == MS_GC_BLACK) {
      ...
      // 垃圾：从链表移除，调用 tpFree，释放内存
      MsObject* dead = cur;
      *prev = dead->gcNext;
      cur   = dead->gcNext;
      if (dead->type && dead->type->tpFree) {
        dead->type->tpFree(dead);
      }
...
- [ ] 无根引用的对象在 `msGCCollect()` 后被释放（`tpFree` 被调用）。
- [ ] 有根引用的对象在 GC 后仍然存活。
...
static MsType stubType = {
  .name = "stub", .instanceSize = sizeof(MsObject), .tpFree = stubFree
};
...
- **`tpFree` vs GC free**：`tpFree` 用于析构（如关闭文件句柄、释放非 GC 堆内存），不负责 `msFree(obj)` 本身（GC sweep 负责 `msFree`）。
