...
- [x] `.gitignore` 包含 `build/`、`__mscache__/`、`*.msc`。 <!-- v:manual:grep 检查 .gitignore 内容 -->
- [x] 根目录存在 `.editorconfig`、`.clang-format`、`.clang-tidy`（`c-style.md §13.1` 要求）。 <!-- v:manual:ls 检查根目录文件 -->
- [x] `tests/ci/build_check.sh` 存在且在 Linux/macOS 下 `bash tests/ci/build_check.sh` 输出 `BUILD OK`。 <!-- v:manual:仅 Linux/macOS 可验证 -->
