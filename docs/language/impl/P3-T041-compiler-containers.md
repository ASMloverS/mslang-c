...
**注**：追加 `OP_GET_SLICE`（取 4 个参数：obj、lo、hi、step，避免创建中间 slice 对象）到操作码枚举更高效；初版可先用 `OP_BUILD_SLICE + OP_GET_INDEX` 实现，后续优化。
...
- [ ] `"()"` → `OP_BUILD_TUPLE(0)`。
- [ ] `"a[1]"` → `OP_GET_LOCAL(a)`, `OP_CONST(1)`, `OP_GET_INDEX`。
- [ ] `"a[1:3]"` → obj + NIL/CONST(1) + CONST(3) + NIL + `OP_BUILD_SLICE` + `OP_GET_INDEX`（或 `OP_GET_SLICE`）。
...
- **超过 65535 个元素**：`emitOp16` 限制 `arg <= 65535`；超出报编译错误（实践中极少见）。
