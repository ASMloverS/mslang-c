...
| 文档 | 章节 |
|---|---|
| `stdlib/stdlib-heapq-queue.md` | §1 模块 API |
...
q.putNowait(item)                // 等同 put(block=false)
q.getNowait() → item            // 等同 get(block=false)
q.taskDone()                    // 通知 join() 一个任务已完成
q.join()                         // 阻塞直到所有任务 taskDone
q.qsize() → int
q.empty() → bool
q.full() → bool
...
queue.Empty                      // 异常：getNowait 空时抛
queue.Full                       // 异常：putNowait 满时抛
```
...
// taskDone/join：unfinished_tasks 计数 + 条件变量
