...
实现通用迭代协议的 VM 层：`OP_GET_ITER`（将 iterable 转为 iterator）、`OP_FOR_ITER`（取下一个元素或跳出循环），以及 `OP_BUILD_SLICE`（构建切片对象供 `OP_GET_INDEX` 使用）。这些指令将所有实现了 `tpIter`/`tpNext` 类型槽的对象统一接入 `for` 循环。
...
case OP_GET_ITER: {
  ...
  MsValue iter = tp->tpIter(iterable);
...
- **`tpNext` 返回 nil 作为 StopIteration 哨兵**：这是内部约定，不影响 mslang 代码中 `nil` 值的正常迭代（list 中的 nil 元素通过 iter 内部计数管理，不会被误识别为耗尽）。
