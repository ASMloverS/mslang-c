...
- [x] `cmake --build build`（含新源文件）编译通过，无警告。 <!-- v:build -->
- [x] `msVecPush` 压入 1024 个 uint64_t 元素后，`len==1024`，`data[1023]` 值正确。 <!-- v:ctest:test_common_utils -->
- [x] `msFnv1a32("hello", 5)` 返回 `0x4f9f2cab`（已知标准值）。 <!-- v:ctest:test_common_utils -->
- [x] `msFnv1a64("hello", 5)` 返回 `0xa430d84680aabd0b`（已知标准值）。 <!-- v:ctest:test_common_utils -->
- [x] `msRealloc(NULL, 64)` 等价 `malloc(64)` 正常返回。 <!-- v:ctest:test_common_utils -->
