...
> **安全点覆盖**：VM 在每次循环回边（`FOR_ITER`/反向 `JMP`）及每次 `CALL` 前均自动检查安全点（见 vm.md §7）。含回边的循环（`for`、`for v in list`、`while`）和所有函数调用均已覆盖，无需用户干预。
>
> **例外**：没有回边、也不含函数调用的超长直线代码段（极少见）会阻塞 GC STW 与调度器公平性，使其他 goroutine 饥饿。初版若遇此情形，需在代码中插入 `go func(){}()` 或 `await` 主动让出。后续版本可加入**基于信号的抢占**（类 Go 1.14 异步抢占）作为兜底。
